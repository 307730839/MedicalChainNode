...
public class TransactionController {
    ...
    @RequestMapping(value = "/index_id", method = RequestMethod.GET)
    public ModelAndView index_id() {
        ModelAndView mav = new ModelAndView("/transaction/index_id");
        return mav;
    }
    @RequestMapping(value = "/searchName", method = RequestMethod.GET)
    public ModelAndView searchName(@RequestParam("txId") String txId) {
        logger.info("txId 为: " + txId);
        Transaction tx1 = txSrv.findByStrainName(txId);
        txId = tx1.getTxId();
