...
public class TransactionService {
    ...
    public Transaction findByStrainName(String strainName) {
        return this.findByStrainName(strainName, JsonUtil.getMongoDBConfig(Const.BlockChainConfigFile), NetUtil.getPrimaryNode());
    }
    public Transaction findByStrainName(String strainName, MongoDBConfig mongoDBConfig, NetAddress netAddress) {
        MongoDB mongoDB = new MongoDB(mongoDBConfig);
        String txCollection = netAddress + "." + Const.TX;
        List<String> list = mongoDB.findName("strainName", strainName, txCollection);
        if (list.size() == 0)
            return null;
        else if (list.size() == 1) {
            Transaction tx = null;
            ...
            logger.error("id 为： " + strainName + " 的tx记录存在多条");
            return null;
        }
    }    /**
