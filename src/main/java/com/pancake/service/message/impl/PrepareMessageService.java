...
public class PrepareMessageService {
    ...
    public static boolean procPMsg(String rcvMsg, NetAddress validatorAddr) throws IOException {
        ...
        String prefix = validatorAddr.toString()  + ".";
        ...
        if (verifyRes) {
            ...
            String pmCollection = null;
            String ppmCollection = null;
            String cmtmCollection = null;
            String runMod = null;
            runMod = JsonUtil.getRunMod(Const.BlockChainConfigFile);
            if(runMod.equals("test")) {
                pmCollection = prefix + Const.PM;
                ppmCollection = prefix + Const.PPM;
                cmtmCollection = prefix + Const.CMTM;

            }
            else{
                pmCollection =  Const.PM;
                ppmCollection =  Const.PPM;
                cmtmCollection =  Const.CMTM;
            }
            ...
            if (2 * PeerUtil.getFaultCount() + 1 <= count) {
                ...
                String pdmCollection = null;
                if(runMod.equals("test")) {
                    pdmCollection = prefix + Const.PDM;
                }
                else{
                    pdmCollection =  Const.PDM;
                }
